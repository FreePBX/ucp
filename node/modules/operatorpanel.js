...
Operatorpanel = function(freepbx) {
	var ami = freepbx.astman,
	    ...
	    jsonqueues = [],
	    jsonparkinglots = [],
	    operatorPanelSocket = io.of(namespace),
	...
	ami.on('managerevent',function(msg){
		if(msg.event != 'VarSet' && msg.event != 'Newexten')
  		console.log(msg);
	});
	...
	*/
	ami.on('extensionstatus', function(evt){
		...
		if(typeof evt.actionid === 'undefined' || evt.actionid === null){
			...
			io.of(namespace).emit('Event-ExtensionStatus',element);
		}
		else{
	...
	ami.on('extensionstatelistcomplete', function(evt){
		...
		io.of(namespace).emit('Event-ExtensionStateListComplete', jsondev[evt.actionid]);
	...
	*/ 
	ami.on('queuestatuscomplete', function(evt){
		...
		io.of(namespace).emit('Event-QueueStatusComplete', jsonqueues[evt.actionid]);
		jsonqueues[evt.actionid] = [];
		queues[evt.actionid] = [];
	});
	...
	*/
	ami.on('peerstatus', function(evt){
		...
		io.of(namespace).emit('Event-PeerStatus', element);
	});
	...
	*/
	ami.on('queuememberstatus', function(evt){
		...
		io.of(namespace).emit('Event-QueueMemberStatus', element);
	});
	...
	*/	
	ami.on('queuememberadded', function(evt){
		var element = {
                ...
                io.of(namespace).emit('Event-QueueMemberAdded', element);
	});
	...
	*/
	ami.on('queuememberremoved', function(evt){
		var element = {
                ...
                io.of(namespace).emit('Event-QueueMemberRemoved', element);
	});	
	...
	*/
	ami.on('parkinglotscomplete', function(evt){
		io.of(namespace).emit('Event-ParkinglotsComplete', jsonparkinglots[evt.actionid]);
	...
	*/
	ami.on('newexten', function(evt){
		...
		io.of(namespace).emit('Event-Newexten', element);
	});
	...
	*/
	ami.on('hangup', function(evt){
		...
		io.of(namespace).emit('Event-Hangup', element);
	});
	...
	*/
	ami.on('parkedcall', function(evt){
		...
		io.of(namespace).emit('Event-ParkedCall', element);
	});
	...
	*/
	ami.on('unparkedcall', function(evt){
		var element = {
                ...
                io.of(namespace).emit('Event-UnparkedCall', element);
	});
	...
	*/
	ami.on('parkedcallswap', function(evt){
		var element = {
                ...
                io.of(namespace).emit('Event-ParkedCallSwap', element);
	});
	...
	*/
	ami.on('parkedcallgiveup', function(evt){
		var element = {
                ...
                io.of(namespace).emit('Event-ParkedCallGiveUp', element);
	});
	...
	*/
	ami.on('parkedcalltimeout', function(evt){
		var element = {
                ...
                io.of(namespace).emit('Event-ParkedCallTimeOut', element);
	});
	...
	*/
	...
	operatorPanelSocket.on('connection', function(socket){
			...
			query.on('end', function(){
				ami.action({
                           		...
                           		'actionid' : actionid
                                	...
                                	io.of(namespace).emit('Action-ExtensionStateList', res); 
				});
			});
		});
		...
		*/
		socket.on('Action-Originate', function(msg){
			ami.action({
				...
				io.of(namespace).emit('Action-Originate', res);
			});
		});
		...
		*/
		socket.on('Action-ChanSpy', function(msg){
			...
			ami.action({
				...
				io.of(namespace).emit('Action-ChanSpy', res);
			}); 
		});
		...
		*/
		socket.on('Action-Monitor', function(msg){
			ami.action({
				...
				io.of(namespace).emit('Action-Monitor', res);
			});
		});
		...
		*/
		socket.on('Action-StopMonitor', function(msg){
			ami.action({
				...
				io.of(namespace).emit('Action-StopMonitor', res);
			});
		});
		...
		*/
		socket.on('Action-PauseMonitor', function(msg){
			ami.action({
				...
				io.of(namespace).emit('Action-PauseMonitor', res);
			});
		});
		...
		*/
		socket.on('Action-UnpauseMonitor', function(msg){
			ami.action({
				...
				io.of(namespace).emit('Action-UnpauseMonitor', res);
			});
		});
		...
		socket.on('Action-QueueStatus', function(msg){
			...
			query.on('end', function(){
				ami.action({
					...
					io.of(namespace).emit('Action-QueueStatus', res);
				});
			});
		});
		...
		*/
		socket.on('Action-Atxfer', function(msg){
			...
			ami.action({
				...
				io.of(namespace).emit('Action-Atxfer', res);
			});
		});
		...
		*/
		socket.on('Action-BlindTransfer', function(msg){
			...
			ami.action({
				...
				io.of(namespace).emit('Action-BlindTransfer', res);
			});
		});
		...
		*/
		socket.on('Action-Parkinglots', function(msg){
			...
			ami.action({
				...
				io.of(namespace).emit('Action-Parkinglots', res);
			});
		});
		...
		*/
		socket.on('Action-Park', function(msg){
			...
			ami.action({
				...
				io.of(namespace).emit('Action-Park', res);
			});
		});
	}); 
};
