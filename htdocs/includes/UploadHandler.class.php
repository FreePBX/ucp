...
class UploadHandler {
    ...
    function __construct($freepbx = null, $options = null, $initialize = true, $error_messages = null) {
        $this->options = [
    ...
    protected function trim_file_name(
        ...
        $name = trim(basename(stripslashes((string) $name)), ".\x00..\x20");
    ...
    protected function is_valid_image_file($file_path) {
        if (!preg_match($this->options['image_file_types'], (string) $file_path)) {
            return false;
        }
        if (function_exists('exif_imagetype')) {
    ...
    protected function generate_response($content, $print_response = true) {
        if ($print_response) {
            ...
            $redirect = isset($_REQUEST['redirect']) ?
                stripslashes((string) $_REQUEST['redirect']) : null;
            if ($redirect) {
    ...
    protected function get_version_param() {
        return isset($_GET['version']) ? basename(stripslashes((string) $_GET['version'])) : null;
    }
    ...
    protected function get_file_name_param() {
        ...
        return isset($_GET[$name]) ? basename(stripslashes((string) $_GET[$name])) : null;
    }

    protected function get_file_names_params() {
        $params = $_GET[$this->options['param_name']] ?? [];
        foreach ($params as $key => $value) {
            $params[$key] = basename(stripslashes((string) $value));
        }
        return $params;
    }
    ...
    public function post($print_response = true) {
        ...
        $file_name = $this->get_server_var('HTTP_CONTENT_DISPOSITION') ?
            rawurldecode(preg_replace(
                '/(^[^"]+")|("$)/',
                '',
                (string) $this->get_server_var('HTTP_CONTENT_DISPOSITION')
            )) : null;
        // Parse the Content-Range header, which has the following form:
        // Content-Range: bytes 0-524287/2000000
        $content_range = $this->get_server_var('HTTP_CONTENT_RANGE') ?
            preg_split('/[^0-9]+/', (string) $this->get_server_var('HTTP_CONTENT_RANGE')) : null;
