...
class User {
	...
	private function _authenticate($username, $password, $directlogin = false) {
		...
		if(!empty($result) && $this->_allowed($result,$directlogin)) {
			$this->uid = $result;
	 ...
	 */
	private function _allowed($uid,$direct = false) {
		// allow templatecreator login using Key
		if($direct){
			return true;
		}
		//template creator has direct login, check uid is matching with template creator
		$tuid = $this->getTemplateCreatorUid();
		//after login all request will hit here with $direct = false
		if($tuid == $uid && $direct == false ) {
			return true;
		}
