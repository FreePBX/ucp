	...
	function getDisplay($dashboard_id) {
		...
		if(!empty($widgets_info)){
			foreach($widgets_info as $data) {
				$html .= '<li data-id="'.$data->id.'" data-name="'.$data->name.'" data-row="'.$data->row.'" data-col="'.$data->col.'" data-sizex="'.$data->size_x.'" data-sizey="'.$data->size_y.'" data-rawname="'.$data->rawname.'" data-widget_type_id="'.$data->widget_type_id.'">
					<div class="widget-title">'.$data->name.' <div class="remove-widget" data-widget_id="'.$data->id.'"><i class="fa fa-times" aria-hidden="true"></i></div></div>
