...
$(document).bind("logIn", function( event ) {
	tour = new Tour({
		...
		keyboard: false,
		onEnd: function (tour) {
			//window.location.replace("http://daltonempire.nl/?tour=false");
		},
		steps: [
			{
				orphan: true,
				...
				onNext: function() {
					var step = tour.getCurrentStep();
					$(document).on("addDashboard",function(e, id) {
						$(".dashboard-menu[data-id="+id+"]").addClass("tour-step");
						$(".dashboard-menu[data-id="+id+"] a[data-pjax]").click();
						tour.goTo(step + 1);
					});
				...
				content: _("Widgets are sorted into categories on the left. You can click on any category to get a listing of the widgets available"),
				previous: -1
			}, {
				element: "#add_widget .bhoechie-tab-content.active .ibox-content-widget:first",
				placement: "bottom",
				title: _("Selecting Widgets"),
				...
				element: "#add_widget .bhoechie-tab-content.active .ibox-content-widget:first .add-widget-button",
				placement: "right",
				title: _("Adding Widgets"),
				...
				element: "#side_bar_content .add-widget",
				placement: "right",
				title: _("Adding sidebar Widgets"),
				...
				onNext: function() {
					var step = tour.getCurrentStep();
					$("#add_widget").one("shown.bs.modal", function() {
						$("#add_widget .modal-body").scrollTop($("#add_widget .modal-body")[0].scrollHeight);
						tour.goTo(step + 1);
					});
				}
			}, {
				element: "#add_widget .list-group-item:last",
				placement: "right",
				title: _("Selecting Small Widgets"),
				content: _("Small widgets are grouped in a single category called 'Side Bar Widgets' which appears at the bottom of the Widget cateogry list")+"<br><br>"+_("Click this symbol to continue"),
				reflex: true,
				next: -1,
				onNext: function() {
					var step = tour.getCurrentStep();
				...
				element: "#add_widget .bhoechie-tab-content.active .ibox-content-widget:first .add-small-widget-button",
				placement: "right",
