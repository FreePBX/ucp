		...
		buildPlayer: function (options) {
			...
			this.each(function (idx) {
				...
				if (!player.opt.m4a && url.indexOf("m4a")>0)
					player.opt.m4a = url;
				if (!player.opt.wav && url.indexOf("wav")>0)
					player.opt.wav = url;
				if( typeof player.opt.mp3 == "undefined")
					player.opt.mp3 = null;
				if( typeof player.opt.m4a == "undefined")
					player.opt.m4a = null;
				if( typeof player.opt.wav == "undefined")
					player.opt.wav = null;
				...
				var fileUrl = encodeURI(player.opt.mp3 || player.opt.m4a || player.opt.wav);
				...
				if(player.opt.wav){
					player.opt.media.wav = player.opt.wav;
					player.opt.supplied.push("wav");
				}
